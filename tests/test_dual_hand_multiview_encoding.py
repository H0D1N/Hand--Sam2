...
def check_chunked_encoding_preserves_batch_view_order():
    ...
    frame_images = torch.tensor([1.0, 2.0, 3.0, 4.0]).reshape(2, 2, 1, 1, 1)

    chunked = model._encode_frame_views(frame_images, encoder_chunk_size=1)
    assert model.chunk_sizes == [1, 1, 1, 1]

    model.chunk_sizes.clear()
    unchunked = model._encode_frame_views(frame_images, encoder_chunk_size=4)
    assert model.chunk_sizes == [4]
    ...
    assert main_feature[:, :, 0].tolist() == [[1.0, 2.0, 3.0, 4.0]]
    assert main_pos_embed[:, :, 0].tolist() == [[51.0, 52.0, 53.0, 54.0]]
    assert high_res["left"][0].flatten().tolist() == [2.0, 4.0, 6.0, 8.0]
    assert high_res["right"][0].flatten().tolist() == [3.0, 6.0, 9.0, 12.0]
...
def check_encoding_keeps_only_required_gradients():
    ...
    frozen_outputs = model._encode_frame_views(frame_images, encoder_chunk_size=1)
    ...
    trainable_outputs = model._encode_frame_views(frame_images, encoder_chunk_size=1)
    ...
    projection_outputs = model._encode_frame_views(frame_images, encoder_chunk_size=1)
...
def check_invalid_encoder_chunk_size():
    model = FakeChunkedEncoder()
    frame_images = torch.ones(1, 2, 1, 1, 1)

    try:
        model._encode_frame_views(frame_images, encoder_chunk_size=0)
    except ValueError as error:
        assert "encoder_chunk_size" in str(error)
    else:
        raise AssertionError("encoder_chunk_size=0 应该报错")


def main():
    check_chunked_encoding_preserves_batch_view_order()
    check_encoding_keeps_only_required_gradients()
    check_invalid_encoder_chunk_size()
